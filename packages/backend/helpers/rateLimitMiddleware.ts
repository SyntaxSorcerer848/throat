...
let rateLimiters = new Map<number, RateLimiterRedis>();

//We can make this dynamic based on the subscription as well
let RATE_LIMIT_DURATION_IN_MINUTES = 1;
let FALL_BACK_DEFAULT_RATE_LIMIT = 100;

let getRateLimiter = (rateLimit: number): RateLimiterRedis => {
    if (!rateLimiters.has(rateLimit)) {
        let opts: IRateLimiterStoreOptions = {
            storeClient: redis,
...
let rateLimitMiddleware = () => async (req: Request, res: Response, next: Function) => {
    if (skipRateLimitRoutes(req)) {
        return next();
    }
    try {
        let { 'x-revert-t-id': tenantId } = req.headers;
        let { subscription, id: accountId } = res.locals.account || {}; // Subscription details are retrieved from response locals set earlier in the revertAuthMiddleware.
        let rateLimit =
        ...
        let rateLimiter = getRateLimiter(rateLimit);
        //added accountId to make the key unique
        let uniqueKey = `${accountId}-${tenantId}`;
